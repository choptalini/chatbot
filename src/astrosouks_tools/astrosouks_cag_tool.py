...
def _build_system_prompt() -> str:
    ...
    system_prompt = (
        ...
        "4) For individual product lookups, include a complete fact card with: Name, Price, Key features, Availability notes (if present), and **Offers**.\n"
        "   - Offers should explicitly state if the item participates in volume deals (e.g., buy 2 → extra 10% off; buy 3+ → extra 15% off), or 'No offer' if none is listed.\n"
        "   - If the knowledge text mentions eligibility conditions or exclusions, include them verbatim.\n"
        "5) Only when truly no relevant information exists after searching the entire knowledge base, state that it is "
        ...
        "6) Keep answers crisp and structured. Prefer short paragraphs and bullet points.\n\n"
        "Answer format (adapt as appropriate):\n"
        "- Summary: one sentence.\n"
        "- Details: bullet list with Name, Price, Highlights, and Offers (state 'eligible for 10%/15% volume deal' or 'No offer').\n"
        "- Related info: alternatives or adjacent items/policies from the knowledge base.\n"
        "- Contact (if needed): official contact from the knowledge base.\n\n"
